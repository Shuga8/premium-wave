...
$('.navbar-search-field').on('input', function () {
  ...
  match.each(function (idx, elem) {
    ...
    var item_text = $(elem).find(".menu-title").text().replace(/(\+)/g, '').trim();
    var badgeText= $(elem).find(`.menu-badge`).length  ? `<small class="text--small text--danger">(${$(elem).find(`.menu-badge`).text()})</small>` : '';

    $(search_result_pane).append(`
        <li>
          ${parent}
          <a href="${item_url}" class="fw-bold text-color--3 d-block">${item_text}${badgeText}</a>
        </li>
      `);
  });
