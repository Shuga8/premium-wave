...
class AdvertisementController extends Controller
{
    ...
    public function create($id = 0)
    {
        ...
        abort_if(!in_array($step, $steps), 404);
        abort_if(!$id && $step != 1, 404);
    ...
    public function stepTwo($request, $id)
    {
        ...
        if ($request->price_type == Status::P2P_AD_PRICE_TYPE_FIXED) {
            ...
            $coinPirce     = @$ad->asset->marketData->price;
            $currencyPirce = @$ad->fiat->rate;
            $price         = (($coinPirce / $currencyPirce) / 100) * $request->margin;
        }
    ...
    public function stepThree($request, $id = 0)
    {
        $request->validate([
            ...
            'auto_reaply_text' => 'nullable|string',
        ]);
        ...
        $ad->auto_reaply_text = $purifier->purify($request->auto_reaply_text);
    ...
    {
        $oldPaymentMthod      = AdPaymentMethod::where('ad_id', $id)->pluck('payment_method_id')->toArray() ?? [];
        $requestPaymentMethod = $request->payment_method;
        $now                  = now();

        $reomvePaymentMethods = array_diff($oldPaymentMthod, $requestPaymentMethod);
        $addPaymentMethods    = array_diff($requestPaymentMethod, $oldPaymentMthod);

        if ($reomvePaymentMethods) {
            AdPaymentMethod::where('ad_id', $id)->whereIn('payment_method_id', $reomvePaymentMethods)->delete();
        }
        ...
        if ($addPaymentMethods) {
            ...
            AdPaymentMethod::insert($paymentMethos);
        }
    }
