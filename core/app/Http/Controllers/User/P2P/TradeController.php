...
class TradeController extends Controller
{
    public function request(Request $request, $id)
    {
        ...
        $feeback           = userFeedabck($ad->user_id);
        $html              = view($this->activeTemplate . "p2p.trade.request.$type", compact('pageTitle', 'ad', 'coinWalletBalance', 'type', 'feeback'))->render();
    ...
    public function requestSave(Request $request, $id)
    {
        ...
        if (@$trade->ad->auto_reaply_text) {
            $message              = new TradeMessage();
            $message->message     = $trade->ad->auto_reaply_text;
    ...
    public function details($id)
    {
        ...
        $feeback                   = userFeedabck($trader->id);
        $tradeFeeback              = TradeFeedBack::where('trade_id', $trade->id)->first();
        $feedBackAbility           = $this->checkFeebackAbility($trade, $user->id);
        ...
        return view($this->activeTemplate . 'user.p2p.trade.details', compact('trade', 'pageTitle', 'user', 'sellerPaymentMethod', 'trader', 'messages', 'feeback', 'tradeFeeback', 'feedBackAbility', 'paymentTimeRemind', 'paymentTimeRemindInSecond'));
    }
    ...
    public function feeback(Request $request, $id)
    {
        $request->validate([
            'type'    => 'required|in:' . Status::P2P_TRADE_FEEDBACK_NEGAVTIVE . ',' . Status::P2P_TRADE_FEEDBACK_POSSITIVE . '',
            'comment' => 'required|string',
        ]);

        $feebackId = $request->feedback_id ?? 0;
        $trade     = Trade::myTrade()->where('id', $id)->firstOrFail();
        $user      = auth()->user();

        if ($feebackId) {
            $feeback = TradeFeedBack::where('trade_id', $trade->id)->where('id', $feebackId)->where('provide_by', $user->id)->firstOrFail();
        } else {
            $feeback = TradeFeedBack::where('trade_id', $trade->id)->first();
            abort_if($feeback || !$this->checkFeebackAbility($trade, $user->id), 404);
            $feeback             = new TradeFeedBack();
            $feeback->trade_id   = $trade->id;
            $feeback->user_id    = $trade->ad->user_id;
            $feeback->provide_by = $user->id;
        }
        $feeback->comment = $request->comment;
        $feeback->type    = $request->type;
        $feeback->save();

        return returnBack("Feedback added successfully", 'success');
    }
    public function feebackDelete($id)
    {
    ...
    private function checkFeebackAbility($trade, $userId)
    {
