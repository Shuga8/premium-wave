                        ...
                        <div class="d-flex gap-2 flex-wrap flex-column text-end">
                            @if ($trade->buyer_id == $user->id && Status::P2P_TRADE_PENDING == $trade->status)
                                <div class="text-start text-md-end mt-2 mt-md-0">
                                    <button class="btn  btn--danger btn--sm outline confirmationBtn"
                                        data-question="@lang('Are you sure to cancel this trade')?"
                                        data-action="{{ route('user.p2p.trade.cancel', $trade->id) }}">
                                        <i class="las la-times-circle"></i> @lang('Cancel')
                                    </button>
                                    <button class="btn  btn--success btn--sm outline confirmationBtn"
                                        data-question="@lang('Are you sure to paid this trade')?"
                            ...
                            @if ($trade->seller_id == $user->id && Status::P2P_TRADE_PAID == $trade->status)
                                <div class="mt-2 mt-md-0">
                                    <button class="btn  btn--danger btn--sm outline confirmationBtn"
                                        data-question="@lang('Are you sure to dispute this trade')?"
                                        data-action="{{ route('user.p2p.trade.dispute', $trade->id) }}">
                                        <i class="las la-times-circle"></i> @lang('Dispute')
                                    </button>
                                    <button class="btn  btn--success btn--sm outline confirmationBtn"
                                        data-question="@lang('Are you sure to release this trade')?"
                ...
                <div class="row">
                    <div class="col-lg-8">
                        <div class="order-details__left">
                            ...
                            @if ($feedBackAbility && !$tradeFeeback)
                                <div class="order-details__feedback">
                                    ...
                                    <div class="order-details__feedback-btn">
                                        <button data-type="{{ Status::P2P_TRADE_FEEDBACK_POSSITIVE }}"
                                            data-class="btn--success" type="button" class="feedback-btn btn--success">
                                            <span class="like">
                                                <i class="las la-thumbs-up"></i></span>
                                            @lang('Positive')
                                        </button>
                                        <button data-type="{{ Status::P2P_TRADE_FEEDBACK_NEGAVTIVE }}"
                                            data-class="btn--danger" type="button" class="feedback-btn">
                                            <span class="like"> <i class="las la-thumbs-down"></i></span>
                                            @lang('Negative')
                                        </button>
                                    </div>
                                    <div class="order-details__feedback-item">
                                        <form method="post" action="{{ route('user.p2p.trade.feeback', $trade->id) }}"
                                            class="feeback-form">
                                            @csrf
                                            <input type="hidden" name="type"
                                                value="{{ Status::P2P_TRADE_FEEDBACK_POSSITIVE }}">
                            ...
                            @if ($tradeFeeback)
                                <div class="order-details__feedback">
                                    <h6 class="order-details__feedback-title mb-4">
                                        @lang('Feedback')
                                        @if (Status::P2P_TRADE_FEEDBACK_POSSITIVE == $tradeFeeback->type)
                                        ...
                                        @endif
                                        <div class="order-details__feedback-btn mt-3 d-none show-hide-feeback">
                                            <button data-type="{{ Status::P2P_TRADE_FEEDBACK_POSSITIVE }}"
                                                data-class="btn--success" type="button"
                                                class="feedback-btn @if ($tradeFeeback->type == Status::P2P_TRADE_FEEDBACK_POSSITIVE) btn--success @endif">
                                                <span class="like">
                                                    <i class="las la-thumbs-up"></i></span>
                                                @lang('Positive')
                                            </button>
                                            <button data-type="{{ Status::P2P_TRADE_FEEDBACK_NEGAVTIVE }}"
                                                data-class="btn--danger" type="button"
                                                class="feedback-btn @if ($tradeFeeback->type == Status::P2P_TRADE_FEEDBACK_NEGAVTIVE) btn--danger @endif ">
                                                <span class="like"> <i class="las la-thumbs-down"></i></span>
                                                @lang('Negative')
                                            </button>
                                        </div>
                                    ...
                                    <div class="order-details__feedback-item d-none feedbackform">
                                        <form method="post" action="{{ route('user.p2p.trade.feeback', $trade->id) }}"
                                            class="feeback-form">
                                            @csrf
                                            <input type="hidden" name="type" value="{{ $tradeFeeback->type }}">
                                            <input type="hidden" name="feedback_id" value="{{ $tradeFeeback->id }}">
                                            <div class="form-group">
                                                <label class="form--label">@lang('Comment')</label>
                                                <textarea class="form-ocntrol form--control" name="comment">{{ $tradeFeeback->comment }}</textarea>
                                            </div>
                                            <button type="button"
                                                class="btn btn--dark edit-cancel">@lang('Cancel')</button>
                                            <button type="submit" class="btn btn--base">@lang('Submit')</button>
                                        </form>
                                    </div>
                                    <div class="order-details__feedback-item">
                                        <h6 class="title mb-2">{{ __($tradeFeeback->comment) }}</h6>
                                        <div class="feedback-edit">
                                            <span class="fs-14">{{ showDateTime($tradeFeeback->created_at) }}</span>
                                            @if ($tradeFeeback->provide_by == $user->id)
                                                <button type="button" class="feedback-edit editBtn">
                                                    <i class="las la-edit"></i>
                                                </button>
                                                <button type="button" class="confirmationBtn"
                                                    data-question="@lang('Are you sure to delete this feedback')?"
                                                    data-action="{{ route('user.p2p.trade.feeback.delete', $tradeFeeback->id) }}">
    ...
    <script>
        ...
        (function($) {
            ...
            $('.feedback-btn').on('click', function(e) {
                ...
                $('.feeback-form').find(`[name=type]`).val($(this).data(`type`));
            });
