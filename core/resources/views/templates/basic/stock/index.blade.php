...
@section('content')
	<div class="trading-section bg-color py-60">
		<div class="custom--container container">
			...
			<!--                                 gsd </a>-->
			<!--                            	<span class="rate-symbol mt-3 text-white" style="font-size: 22px;">IBM/USD</span>-->
			<!--                         </div>-->
			<!--                     </div>-->
			<!--                 </div>-->
			<!--             </div>-->
			...
			<!--                             <a class="dashboard-card__coin-name mb-0" href="{{ route('user.order.open') }}">-->
			<!--                                 gsd </a>-->
			<!--                             <h6 class="dashboard-card__coin-title"> fhfh </h6>-->
			<!--                         </div>-->
			<!--                     </div>-->
			<!--                 </div>-->
			<!--             </div>-->
			...
			<!--             </div>-->
			<!--              </div>-->
			<div class="commodites-page mt-2">
				<div class="col">
					<x-flexible-view :view="$activeTemplate . 'stock.orders'" :meta="['pair' => $pair]" />
				</div>
						...
						<form id="binary-form" name="limitForm" action="" method="POST">
							...
							<div class="d-flex form-group gap-2">
								<button class="btn btn-danger w-100 set-btn border-0 text-white" type="button"
									style="color:white !important;">Sell <span class="buy_limit"></span></button>
							...
							{{-- <div class="form-group">
								<button class="btn btn-primary w-100 px-5 py-3 text-white" id="set-btn">Set</button>
							</div> --}}
						</form>
					</div>
				</div>
				...
				<div class="col markets-col" style="  border-radius:5px;">
					<div class="right" style="display:flex; flex-direction:column;">
						<div class="market">
							...
							<ul class="commodities--list">
								...
								@foreach ($stocks as $stock)
									<li class="{{ $stock->id == 1 ? 'active-list' : '' }}" data-value="{{ strtoupper($stock->symbol) }}">
										<span class="">{{ strtoupper($stock->symbol) }} / USD</span>
						...
						<x-flexible-view :view="$activeTemplate . 'stock.history'" :meta="['pair' => $pair]" />
					</div>
				</div>
	...
	@push('script')
		<script>
			document.addEventListener('DOMContentLoaded', function() {
				const stocks = @json($stocks->pluck('symbol')); // Assuming $stocks is a collection
		...
		<script>
			function fetchDataAndRender() {
				...
				xhr.open("GET", "{{ route('stock.all') }}", true);
				...
				xhr.onload = function() {
					if (xhr.status == 200) {
						...
						limits.forEach((limit) => {
							...
							let link = limit['status'] == 0 ? `<a class="bg-danger text-white" href="{{ config('app.url') }}stock/cashout/${limit['id']}"
                                    style="padding: 4px;">Cash out</a>` : `<a class="bg-success text-white"
                                    style="cursor: pointer;padding: 4px">Cashed</a>`;
							tr.innerHTML = `
                    <td class="text-success" style="font-size: 15px;">${limit['stock']}</td>
